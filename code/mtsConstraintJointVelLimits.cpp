...
#include <constraintControllerLite/mtsConstraintJointVelLimits.h>
...
void mtsConstraintJointVelLimits::PrepareDataForBridging(mtsInterfaceProvided* provided, std::map<std::string, std::type_index>& bridge_map)
{
    std::string cmd_name;
    ...
    */
    ...
    cmd_name = "_active";
    provided->AddCommandWrite(&mtsConstraintJointVelLimits::set_active, this, Name+cmd_name);
    bridge_map.emplace(Name+cmd_name, std::type_index(typeid(bool)));

    cmd_name = "_lower_limit";
    provided->AddCommandWrite(&mtsConstraintJointVelLimits::set_lower_limits, this, Name+cmd_name);
    bridge_map.emplace(Name+cmd_name, std::type_index(typeid(vctDoubleVec)));

    cmd_name = "_upper_limit";
    provided->AddCommandWrite(&mtsConstraintJointVelLimits::set_upper_limits, this, Name+cmd_name);
    bridge_map.emplace(Name+cmd_name, std::type_index(typeid(vctDoubleVec)));
}
